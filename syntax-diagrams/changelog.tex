...
\item \textbf{configure}: fixed unwanted flags not being removed completely (\bug{627}).
...
\item \textbf{Jobs (-j compilation flag)}: fixed \bug{618}.
...
\begin{itemize}
\item \textbf{Deleted compiler configuration options}: select-working (see \bug{421}).
\item \textbf{New compiler configuration options}: align-opt, align-record, assign-variable, assign-using-variable, assign-ext-dyn, assign-disk-from, dpc-in-data, implicit-assign-dynamic-var, reference-bounds-check, same-as-clause, sync-left-right and vsam-status.
...
\begin{itemize}
\item \textbf{New Micro Focus directives}: DPC-IN-DATA.
...
\begin{itemize}
\item \textbf{SYNCHRONIZED clause}: LEFT phrase is now marked as unimplemented (see \bug{512}).
\end{itemize}
...
\item \textbf{INSPECT statement}: fixed multiple TALLYING phrases being rejected (see \bug{47})
\item \textbf{Procedure division header}: fixed error when a variable-length parameter was not referenced (\bug{648}).
\item \textbf{Procedure division header}: added recognition of z/OS-PARM-style main program parameter (\fr{357}).
...
\item \textbf{SET statement}: added support for FH-{}-FCD and FH-{}-KEYDEF (\fr{349}).
\item \textbf{START statement (transaction)}: added recognition of.
\item \textbf{STRING statement}: strengthened syntax checks (bugs \bugnoname{653} and \bugnoname{488}).
\item \textbf{Tracing}: fixed line numbers when tracing inside RECURSIVE programs (\bug{611}).
\item \textbf{UNSTRING statement}: strengthened syntax checks (bugs \bugnoname{653} and \bugnoname{488}).
\item \textbf{WRITE statement (indexed)}: fixed bug involving file status 21.
\end{itemize}

\section{GnuCOBOL 3.1-dev}

NB: Some changes have been made to 3.1-dev which have not yet been merged up to 4.0-early-dev. They have been marked with an asterisk (*).
...
\item \textbf{cobcrun}: allow redirection of cobcrun's output (\bug{616}).
...
\item \textbf{cobcrun command-line options}: expanded information in --info about the used curses library (\fr{385}).
\item \textbf{COBOL-85 NIST testsuite}: tests can now be downloaded from Sourceforge when the NIST website is unavailable (\bug{563}).
...
\item \textbf{Listings}: fixed segfault in listing programs with large REPLACE arguments (\bug{515}).
\item \textbf{Listings}: fixed stack overflow in listing programs long literals (\bug{569}).
\item \textbf{Mac OS support}: some build issues fixed (see \bug{644}).
...
\item \textbf{National literals}: added Fujitsu NC national character literals (\fr{352}).
\item \textbf{National literals}: fixed NX type literals being treated as alphanumeric (\bug{658}).
\item \textbf{Nested programs}: fixed \bug{435}, where identifiers in the containing program where not propagated to the contained program, causing a segfault.
\item \textbf{Nested programs}: fixed \bug{587}, where a memory error occurred when a program was nested inside a program with the same name.
...
\item  \textbf{Screen section testsuite}: added a manual testsuite for screen section functionality (\patch{26}).
...
\item \textbf{New compiler configuration options}: assign-disk-from, assign-ext-dyn, assign-using-variable, assign-variable, binary-comp-1, call-convention-mnemonic, call-convention-linkage, continue-after, display-special-fig-consts, free-redefines-position (\fr{211}), hp-octal-literals, implicit-assign-dynamic-var, goto-entry, line-col-zero-default, national-character-literals move-figurative-space-to-numeric, move-non-numeric-lit-to-numeric-is-zero, missing-statement, numeric-pointer, perform-without-varying-by, record-delimiter, record-delim-with-fixed-recs, records-mismatch-record-clause, report-column-plus, screen-section-rules, sequential-delimiters, symbolic-constant, type-to-clause*, usage-type*, xml-generate-extra-phrases, zero-length-literals.
\item \textbf{New runtime configuration options}:
  \begin{itemize}
...
\item \textbf{Runtime configuration options}: use the system's file path separator instead of always using : (so the path separator is now ; on Windows). (See \bug{610}.)
\end{itemize}

\paragraph{Compiler directives}
\begin{itemize}
\item \textbf{COPY statement}: added support for copybook and library names containing periods as text-names. For example,  ``COPY copybook.cpy.'' is now accepted. See \fr{344}.
\item \textbf{DISPLAY directives}: allow a wider range of characters.
\item \textbf{@OPTIONS directive}: added recognition of.
\item \textbf{New Micro Focus directives}: ADDRSV, ADDSYN, MAKESYN, OVERRIDE and REMOVE (\fr{210}); ASSIGN, BOUND*, CALLFH, COMP1 and SSRANGE*.
...
\item \textbf{ASSIGN clause}: strengthened syntax checks (\bug{553}).
\item \textbf{ASSIGN clause}: fixed \bug{571} involving external-file-names.
\item \textbf{ASSIGN clause}: added Micro Focus' DISK FROM phrase.
\item \textbf{CALL-CONVENTION phrase}: added C and PASCAL (see \fr{311}).
\item \textbf{CLASS phrase}: added recognition of ALPHANUMERIC\slash{}NATIONAL and alphabet phrases.
\item \textbf{CLASS phrase}: fixed \bug{566}, where class-names were not inherited by nested subprograms.
...
\item \textbf{CURSOR phrase}: fixed \bug{579}, where the CURSOR data item was not used for the initially positioning the cursor in an ACCEPT statement.
...
\item \textbf{FILE STATUS clause}: strengthened syntax checks (\bug{656}).
...
\item \textbf{SELECT entry}: fixed \bug{619}.
...
\item \textbf{ANY LENGTH clause}: fixed \bug{511}, where comparisons treated ANY LENGTH items as having just one character.
...
\item \textbf{errno}: can now access the C global variable errno by declaring a variable as EXTERNAL AS "errno" (\fr{49}).
\item \textbf{LIKE clause*}: added recognition of (see \fr{193}).
...
\item \textbf{PICTURE clause*}: fixed compiler crash with malformed picture-string (\bug{659}).
\item \textbf{RECORD VARYING clause}: fixed \bug{441}, where an FD could be declared as RECORD VARYING, but have records of only one size.
\item \textbf{RENAMES clause}: strengthened syntax checks.
\item \textbf{RENAMES clause}: fixed \bug{623}, where RENAMES items had a LENGTH OF zero.
\item \textbf{SAME AS clause}: implemented (\fr{193}).
...
\item \textbf{TYPEDEF clause*}: implemented (\fr{193}).
...
\item \textbf{USAGE clause}: added experimental support for ACUCOBOL's numeric POINTER (\fr{376}).
...
\item \textbf{ACCEPT statement (screen)}: removed silent minimum of 500~ms from TIME-OUT phrase (\fr{196}).
...
\item \textbf{CALL statement}: added LINKAGE phrase (\fr{311}).
\item \textbf{CALL statement*}: fixed RETURN-CODE being set with RETURNING field (\bug{651}).
\item \textbf{CANCEL statement}: fixed segfault in recursive programs (\bug{98}).
\item \textbf{Conditions}: strengthened syntax checks for abbreviated conditions.
\item \textbf{Conditions}: improved error messages (\bug{567}).
\item \textbf{Conditions}: fixed LESS OR EQUAL being accepted, but LESS OR EQUALS being rejected (\bug{663}).
\item \textbf{CONTINUE (after)}: added support for (from COBOL~202X draft standard). See \fr{354}.
...
\item \textbf{ENTRY statement}: added LINKAGE phrase (\fr{311}).
...
\item \textbf{Error messages}: fixed \bug{586}, where GnuCOBOL would cause a buffer overflow when outputting an error message with a very long literal.
...
\item \textbf{File I-O (ISAM)}: fixed \bug{589}, where READ would skip records with duplicate keys after a DELETE.
...
\item \textbf{JSON GENERATE statement}: added (\patch{45}).
...
\item \textbf{MOVE statement}: fixed remaining cases of \bug{220}, involving numeric-edited items.
...
\item \textbf{Procedure division header}: added LINKAGE phrase (\fr{311}).
\item \textbf{Procedure division header}: error if two USING BY REFERENCE parameters are the same (\bug{635}).
\item \textbf{Procedure division header*}: fixed use of variable-length RETURNING item (\bug{641}).
\item \textbf{Procedure division header*}: disabled RETURNING in programs, pending fix of \bug{650}.
...
\item \textbf{SORT statement}: added sorting of tables without keys (\patch{42}).
...
\item \textbf{OPEN statement}: fixed segfault when attempting to open a file in a non-existent folder (\bug{520}).
...
\item \textbf{STRING statement}: fixed \bug{543}.
\item \textbf{STRING statement}: strengthened syntax checks (\bug{653}).
\item \textbf{SUBTRACT statement}: fixed \bug{603}, where subtracting an integer from a floating-point variable lead to incorrect results.
\item \textbf{STOP statement}: fixed the NORMAL phrase causing a compilation error (\bug{433}).
\item \textbf{UNSTRING statement}: strengthened syntax checks (\bug{653}).
\item \textbf{USE statement (exception)}: improved recognition of.
\item \textbf{VALIDATE statement}: added recognition of.
\item \textbf{WRITE statement}: fixed compilation error when writing to a GLOBAL file in a subprogram (\bug{87}).
\item \textbf{WRITE statement (indexed)}: now sets file status to 02 when duplicate key is detected.
\item \textbf{XML GENERATE statement}: added (\patch{41}).
...
\item \textbf{CBL\_GET\_CSR\_POS}: fixed wrong position being returned after ACCEPT statement (\bug{483}).
...
\item \textbf{X"91"}: corrected number of arguments from 2 to 3 (\bug{527}).
\end{itemize}
